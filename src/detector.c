...
{
    list *options = read_data_cfg(datacfg);
    printf("read_data_cfg done\n");
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    char **names = get_labels_custom(name_list, &names_size); //get_labels(name_list);
    printf("get_labels_custom done\n");
    //image **alphabet = load_alphabet();
    network net = parse_network_cfg_custom(cfgfile, 1, 1); // set batch=1
    printf("parse_network_cfg_custom done\n");
    if (weightfile) {
        load_weights(&net, weightfile);
    }
    ...
    fuse_conv_batchnorm(net);
    printf("fuse_conv_batchnorm done\n");
    calculate_binary_weights(net);
    printf("calculate_binary_weights done\n");
    if (net.layers[net.n - 1].classes != names_size) {
        printf("\n Error: in the file %s number of names %d that isn't equal to classes=%d in the file %s \n",
            name_list, names_size, net.layers[net.n - 1].classes, cfgfile);
    ...
    while (1) {
        ...
        print_detections_v3(im, dets, nboxes, thresh, names, l.classes);
