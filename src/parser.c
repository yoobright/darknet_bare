...
my_buffer load_all_data(char *filename) 
{
    long filelen;
    // char * buffer;
    ...
    rewind(fp);
    my_buffer buffer;
    buffer.data = (char*) malloc(filelen * sizeof(char));
    // buffer.data = yolo_data_buffer;
...
int read_from_buffer(void *data_ptr, size_t __size, size_t __n,
       my_buffer* buffer) 
{
    printf("read_from_buffer: %ld\n", buffer->offset);
    // data_ptr = (char*) malloc(__size * sizeof(char));
    long read_size = __size * __n;
    printf("read_size: %ld\n",  read_size);
    memcpy((char*) data_ptr, (char*) buffer->data + buffer->offset,  read_size);
    buffer->offset =  buffer->offset + read_size;

    return read_size;
}

void load_buffer_convolutional_weights(layer l, my_buffer* buffer)
{
    if(l.binary){
        //load_convolutional_weights_binary(l, fp);
        //return;
    ...
    int read_bytes;
    printf("l.biases\n");
    read_bytes = read_from_buffer(l.biases, sizeof(float), l.n, buffer);
    if (read_bytes > 0 && read_bytes < l.n) printf("\n Warning: Unexpected end of wights-file! l.biases - l.index = %d \n", l.index);
    //fread(l.weights, sizeof(float), num, fp); // as in connected layer
    if (l.batch_normalize && (!l.dontloadscales)){
        printf("l.scales\n");
        read_bytes = read_from_buffer(l.scales, sizeof(float), l.n, buffer);
        if (read_bytes > 0 && read_bytes < l.n) printf("\n Warning: Unexpected end of wights-file! l.scales - l.index = %d \n", l.index);
        printf("l.rolling_mean\n");
        read_bytes = read_from_buffer(l.rolling_mean, sizeof(float), l.n, buffer);
        if (read_bytes > 0 && read_bytes < l.n) printf("\n Warning: Unexpected end of wights-file! l.rolling_mean - l.index = %d \n", l.index);
        printf("l.rolling_variance\n");
        read_bytes = read_from_buffer(l.rolling_variance, sizeof(float), l.n, buffer);
        if (read_bytes > 0 && read_bytes < l.n) printf("\n Warning: Unexpected end of wights-file! l.rolling_variance - l.index = %d \n", l.index);
        if(0){
            int i;
            for(i = 0; i < l.n; ++i){
    ...
    printf("l.weights\n");
    read_bytes = read_from_buffer(l.weights, sizeof(float), num, buffer);
    if (read_bytes > 0 && read_bytes < l.n) printf("\n Warning: Unexpected end of wights-file! l.weights - l.index = %d \n", l.index);
    //if(l.adam){
    //    fread(l.m, sizeof(float), num, fp);
    //    fread(l.v, sizeof(float), num, fp);
...
void load_weights_buffer_upto(network *net, my_buffer* buffer, int cutoff)
{
    ...
    int minor;
    int revision;
    read_from_buffer(&major, sizeof(int), 1, buffer);
    read_from_buffer(&minor, sizeof(int), 1, buffer);
    read_from_buffer(&revision, sizeof(int), 1, buffer);
    // printf("load_weights_buffer_upto: %d, %d, %d\n", major, minor, revision);
    
    if ((major * 10 + minor) >= 2) {
        printf("\n seen 64");
        uint64_t iseen = 0;
        read_from_buffer(&iseen, sizeof(uint64_t), 1, buffer);
        *net->seen = iseen;
    }
    else {
        printf("\n seen 32");
        uint32_t iseen = 0;
        read_from_buffer(&iseen, sizeof(uint32_t), 1, buffer);
        *net->seen = iseen;
    }
...
{
    my_buffer buffer = load_all_data(filename);
    load_weights_buffer_upto(net, &buffer, net->n);
